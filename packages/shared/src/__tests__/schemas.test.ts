...
describe("GameStatusSchema", () => {
    describe("valid inputs", () => {
        it.each(["backlog", "playing", "beaten"])(
            "should accept '%s'",
            (status) => {
...
describe("GameSchema", () => {
    ...
    describe("valid inputs", () => {
        ...
        it("should accept rating at boundary values 1 and 5", () => {
            expect(GameSchema.safeParse({ ...validGame, rating: 1 }).success).toBe(
                true,
            );
            expect(GameSchema.safeParse({ ...validGame, rating: 5 }).success).toBe(
                true,
            );
        });
        ...
        it("should accept a negative statusOrder (re-ordering edge case)", () => {
            expect(
                GameSchema.safeParse({ ...validGame, statusOrder: -1 }).success,
            ).toBe(true);
        });
    });
...
describe("IgdbGameSchema", () => {
    ...
    describe("valid inputs", () => {
        ...
        it("should accept rating at IGDB boundary values 0 and 100", () => {
            expect(
                IgdbGameSchema.safeParse({ ...validIgdbGame, rating: 0 }).success,
            ).toBe(true);
            expect(
                IgdbGameSchema.safeParse({ ...validIgdbGame, rating: 100 }).success,
            ).toBe(true);
        });
    });
...
describe("IgdbSearchResultSchema", () => {
    ...
    describe("invalid inputs", () => {
        it("should reject a non-array value", () => {
            expect(
                IgdbSearchResultSchema.safeParse({ id: 1, name: "Game", slug: "game" })
                    .success,
            ).toBe(false);
        });
...
describe("OutboxEntrySchema", () => {
    ...
    describe("valid inputs", () => {
        ...
        it("should parse a synced entry", () => {
            expect(
                OutboxEntrySchema.safeParse({ ...validEntry, synced: true }).success,
            ).toBe(true);
        });
    });
...
describe("RecommendationSchema", () => {
    ...
    describe("valid inputs", () => {
        ...
        it("should accept matchScore at boundary values 0 and 100", () => {
            expect(
                RecommendationSchema.safeParse({ ...validRec, matchScore: 0 }).success,
            ).toBe(true);
            expect(
                RecommendationSchema.safeParse({ ...validRec, matchScore: 100 })
                    .success,
            ).toBe(true);
        });
    });
